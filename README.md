...
Note, we expect the base model to perform better than this, since this one was trained on less sequences and does not predict for the SPROUT dataset. For simplicity, the script uses the model created from the first repeat, as from our experience, CROP achieved similar results on most datasets. The rest of the models are saved in  `/saved_models_vs_sota/repeats_2-5`.
